...
export default class Robot {
  ...
  get lastKnownCoordinates(): Coordinates {
  ...
  move(): void {
    ...
    if (
      this.area.withinBounds(newCoordinates) ||
      !this.area.scents.has(JSON.stringify(this.coordinates))
    ) {
  ...
  private processor: {
    ...
    L: (c: Coordinates, o: Orientation) => [c, Robot.rotate(o, -1)],
    R: (c: Coordinates, o: Orientation) => [c, Robot.rotate(o, 1)],
    F: (c: Coordinates, o: Orientation) => [Robot.stepTowards(c, o), o],
  };

  private static stepTowards(
    { x, y }: Coordinates,
    orientation: Orientation,
    modifier = 1
  ): Coordinates {
  ...
  private static rotate(
    ...
    // Shortcut to get -/+ index, unfortunately Array#at is not yet supported by TS.
    return compass.slice((currentOrientation + modifier) % 4)[0];
  }
}
