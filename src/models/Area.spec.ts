...
describe('Area', () => {
  it('should throw and error if the size is set above the maximum', () => {
    ...
    expect(() => area.setDimensions({width: 100, height: 100})).toThrowError('Grid height / width too large. The max width is 50 / height is 50.');
  });

  it('should construct with the correct dimension', () => {
    const area = new Area(5, 5, 50, 50);
    expect(area.getDimensions()).toStrictEqual({width: 5, height: 5});
  });


  it('Area#getDimensions should set the dimensions correctly', () => {
    const area = new Area(10, 10, 50, 50);
    expect(area.getDimensions()).toStrictEqual({width: 10, height: 10});
  });
  ...
  it('Area#purge reset all values', () => {
    ...
    expect(area.getDimensions()).toStrictEqual({width: 0, height: 0});
  });
